...
def generate_data(samples=1000):
    ...
    Y = np.zeros((samples, image_size, image_size))  # Zerowa macierz
    center = image_size // 2
    Y[:, center, center] = 1  # Jeden pik w środku obrazu
    return X, Y
...
class THzPropagationLayer(Layer):
    ...
    def call(self, inputs):
        ...
        return tf.stack([tf.math.real(output), tf.math.imag(output)], axis=-1)
